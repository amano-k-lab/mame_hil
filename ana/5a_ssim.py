...
def compute_ssim(image1, image2):
    img1_unit8 = (image1 * 255).astype(np.uint8)
    img2_unit8 = (image2 * 255).astype(np.uint8)
    ssim_scaler, _ = cv2.quality.QualitySSIM_compute(img1_unit8, img2_unit8)
    return ssim_scaler[0]


def generate_and_calculate_ssim(cond_layer, threshold, ecc, model, device, transform, num_images=30, seed=42):
    random.seed(seed)
    cond = Condition(
        ecc=ecc,
        mode="ica",
    ...
    for result in results:
        ...
        ssim_value = compute_ssim(orig_gray, fake_gray)
...
if __name__ == "__main__":
    ...
    ssim_results = {layer: {} for layer in thresholds.keys()}

    for ecc_idx, ecc in enumerate(ecc_values):
        for layer in thresholds.keys():
            threshold = thresholds[layer][ecc_idx]
            ssim_mean, ssim_std = generate_and_calculate_ssim(
                layer, threshold, ecc, model, device, transform, num_images=NUM_IMAGES
            )
            ssim_results[layer][ecc] = {"mean": ssim_mean, "std": ssim_std}
            print(
                f"Layer: {layer}, ECC: {ecc}, Threshold: {threshold}, SSIM Mean: {ssim_mean:.4f}, SSIM Std: {ssim_std:.4f}"
            )
    ...
    for layer, ecc_data in ssim_results.items():
    ...
    plt.tight_layout()
    plt.savefig("output/ana/ssim.png")
