...
if __name__ == "__main__":
    ...
    # Create 4x3 grid plot
    fig, axes = plt.subplots(nrows=8, ncols=3, figsize=(18, 32), sharex=True, sharey=True)
    ...
    for i, subject in enumerate(subjects):
        for j, layer in enumerate(layers):
            ax = axes[i, j]
            ax.set_title(f"{layer}", fontsize=22) if i == 0 else None
            ax.set_ylabel("Mean Threshold", fontsize=22) if j == 0 else None
            ax.set_xlabel("ecc", fontsize=22) if i == 7 else None
            ax.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("output/ana/plot_values/1.a.plot_8x3_subjects_layers_log.png")
    plt.close()
