...
def save_images(image_gen_result: ImageGenResult, output_dir: Path):
    ...
    for i, component_results in enumerate(image_gen_result.examples):
        for key, img_tensor in component_results.items():
            ...
            # file_name = f"robust_{layer_name}_component{i}_plus{value}.png"
            # file_name = f"default_{layer_name}_component{i}_plus{value}.png"
            file_name = f"stylized_{layer_name}_component{i}_plus{value}.png"
...
def generate_selected_component_images(
    data_loader,
    layer_names,
    activation_manager,
) -> ImageGenResult:
    ...
    for layer_name in layer_names:
        ...
        # handler = ICAHandler(Path(config["ica"]["ica_models_default"]) / f"{layer_name}.pkl")
        handler = ICAHandler(Path(config["ica"]["ica_models_stylized"]) / f"{layer_name}.pkl")
        ...
        coordinates.append(coordinates_layer[0])  # comp0 only
        ...
        cond = Condition(
            ecc=3, mode="ica", layer=layer_name, component=0, direction="plus"
        )
...
if __name__ == "__main__":
    ...
    image_gen_result = generate_selected_component_images(
        data_loader=data_loader,
        layer_names=LAYERS,
        activation_manager=activation_manager,
    )

    output_directory = Path("output/figures/model_compare")
